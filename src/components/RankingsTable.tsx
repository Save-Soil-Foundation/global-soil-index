    ...
    <section id="rankings" className="scroll-mt-28">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0">
          <h1 className="gsi-display text-[26px] leading-[1.05] text-white sm:text-[30px] lg:whitespace-nowrap lg:text-[33px]">
            Explore{" "}
            <span className="text-[#8cbf60]">the world&apos;s soil health</span>
          </h1>
          <p className="mt-2 text-sm text-white/48">196 Countries. 1 Global Index.</p>
        </div>

        <div className="flex w-full min-w-0 gap-2 sm:w-auto">
          <div className="relative min-w-0 flex-1 sm:w-[338px] sm:flex-none">
            <input
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              placeholder="Search countries..."
              className="h-10 w-full rounded-full border border-white/18 bg-[#0f171a] pl-4 pr-20 text-sm text-white outline-none transition placeholder:text-white/34 hover:border-white/28 focus:border-[#8cbf60]/70 focus:ring-2 focus:ring-[#8cbf60]/15"
          ...
          <Popover open={filtersOpen} onOpenChange={setFiltersOpen}>
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                className={cn(
                  "h-10 rounded-full border-white/18 bg-[#0f171a] px-4 text-xs text-white/74",
      ...
      <div className="mt-3 grid grid-cols-2 gap-[7px] sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-8 xl:grid-cols-10">
        {visibleCountries.map((country, index) => (
          <Link
            key={country.slug}
            href={`/country/${country.slug}`}
            className="group flex min-h-[124px] flex-col items-center justify-center rounded-md border border-white/[0.055] bg-[#192226] px-2 py-3 text-center transition hover:border-[#8cbf60]/45 hover:bg-[#1d292e] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#8cbf60]"
          >
            <CountryFlag
              country={country}
              width={52}
              ...
              priority={index < 10}
            />
            <div className="mt-2 text-[13px] font-semibold leading-none tabular-nums text-[#8cbf60]">
              {formatRank(country.rank)}
            </div>
            <h2 className="mt-1 flex min-h-[28px] max-w-full items-center text-balance text-[11px] font-semibold leading-[14px] text-white">
              {country.name}
            </h2>
            <div className="mt-1 text-[11px] font-medium tabular-nums text-white/80">
