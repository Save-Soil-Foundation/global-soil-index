...
export const datasetMetadata: DatasetMetadata = {
  id: "gsi-visual-demo-2026-05-26",
  title: "Global Soil Index visual demonstration dataset",
  description:
...
function buildRanking([name, code, region]: CountrySeed): CountryRanking {
  ...
  return {
    ...
    historicalChange,
    dataAvailability,
    verificationStatus: fixtureByName.has(name) ? "visual-demo-fixture" : "modelled-demo",
...
export const tickerItems: SoilTickerItem[] = [
  ...
  ...featuredCountries
    .filter((country) => !tickerReference.some((item) => item.name === country.name))
    .slice(0, 14)
